import { PostStore, postStores } from 'stores/PostStore'
...
function fillProcessingPostStores() {
  const processingPostStores: { [ledgerName: string]: PostProcessingStore } = {}

  Object.keys(postStores).map((ledgerName) => {
    const store = postStores[ledgerName]
    const statusStore = new PostStatusStore(store)
    processingPostStores[ledgerName] = createPostProcessingStore(
      store,
      statusStore
    )
  })
  return processingPostStores
}

export const processingPostStores = fillProcessingPostStores()
